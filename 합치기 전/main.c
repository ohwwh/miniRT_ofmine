	...
	for (int y = HEIGHT - 1; y >= 0; --y)
	{
		...
		for (int x = 0; x < WIDTH; ++x)
		{
			...
			for (int s = 0; s < vars.scene.anti; s ++)
			{
				...
				//lower_left_corner
				u = (((double)x + random_double(0, 2, vars.scene.anti)) * 2 / WIDTH) - 1;
				v = (((double)y + random_double(0, 2, vars.scene.anti)) * 2 / HEIGHT) - 1;
...
int	main(int argc, char *argv[])
{
	...
	t_vec test = unit_vec(vec_sub(create_vec(0,2,0), create_vec(26,8,6)));
