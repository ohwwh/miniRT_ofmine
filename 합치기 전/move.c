...
int key_hook_move(t_minirt* vars)
{
	...
	{
		if (vars->is_move == 13){
			t_vec dir = vec_sub(vars->scene.camera.dir, vars->scene.camera.origin);
			t_vec new_org = vec_sum(vars->scene.camera.origin, micro_vec(dir));
			t_vec new_ = vec_sum(vars->scene.camera.dir, micro_vec(dir));
			vars->scene.camera = create_camera(new_org, new_, vars->scene.camera.vup, vars->scene.camera.fov, vars->scene.camera.ratio);
			...
			t_vec new_ = vec_sum(vars->scene.camera.dir, micro_vec(a_dir));
			vars->scene.camera = create_camera(new_org, new_, vars->scene.camera.vup, vars->scene.camera.fov, vars->scene.camera.ratio);
			set_camera(&vars->scene.camera);
			path_render(*vars);
		}
		else if (vars->is_move == 1){
			t_vec dir = vec_sub(vars->scene.camera.origin, vars->scene.camera.dir);
			t_vec new_org = vec_sum(vars->scene.camera.origin, micro_vec(dir));
			t_vec new_ = vec_sum(vars->scene.camera.dir, micro_vec(dir));
			vars->scene.camera = create_camera(new_org, new_, vars->scene.camera.vup, vars->scene.camera.fov, vars->scene.camera.ratio);
			set_camera(&vars->scene.camera);
			...
			t_vec new_ = vec_sub(vars->scene.camera.dir, micro_vec(a_dir));
			vars->scene.camera = create_camera(new_org, new_, vars->scene.camera.vup, vars->scene.camera.fov, vars->scene.camera.ratio);
			set_camera(&vars->scene.camera);
			path_render(*vars);
		}
		...
		{
			t_vec new_ = create_vec(vars->scene.camera.dir.x,
			vars->scene.camera.dir.y * cos(0.5) - 
			vars->scene.camera.dir.z * sin(0.5),
			vars->scene.camera.dir.y * sin(0.5) + vars->scene.camera.dir.z * cos(0.5));
			vars->scene.camera = create_camera(vars->scene.camera.origin, new_, vars->scene.camera.vup, vars->scene.camera.fov, vars->scene.camera.ratio);
			set_camera(&vars->scene.camera);
			path_render(*vars);
		}
		else if (vars->is_move == 123)
		{
			t_vec new_ = create_vec(vars->scene.camera.dir.z * sin(0.5) + 
			vars->scene.camera.dir.x * cos(0.5), 
			vars->scene.camera.dir.y,
			vars->scene.camera.dir.z * cos(0.5) - vars->scene.camera.dir.x * sin(0.5));
			vars->scene.camera = create_camera(vars->scene.camera.origin, new_, vars->scene.camera.vup, vars->scene.camera.fov, vars->scene.camera.ratio);
			set_camera(&vars->scene.camera);
			path_render(*vars);
		}
		...
		else if (vars->is_move == 4 || vars->is_move == 5)
		{
			...
			if (vars->is_move == 4)
				...
				new_fov = vars->scene.camera.fov + 10;
			vars->scene.camera = create_camera(vars->scene.camera.origin, vars->scene.camera.dir, 
